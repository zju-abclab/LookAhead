...
parser.add_argument("-q", "--quiet", nargs="?", default=False, help="Silence output.")
...
def analyze_contract(
	...
	try:
		...
		contract_msg = (
			"{}: {:.36} completed in {:.2f} + {:.2f} + {:.2f} + {:.2f} secs.".format(
				index,
				contract_name,
...
def run_gigahorse(args, fact_generator: AbstractFactGenerator) -> None:
	...
	res_list = batch_analysis(
		fact_generator, souffle_clients, other_clients, contracts, args.jobs
	)
