...
from src.common import (
	public_function_signature_filename,
	event_signature_filename,
	error_signature_filename,
)
...
def generate_interface():
	...
	for opname, opcode in opcodes.OPCODES.items():
		if opcode.is_push():
			...
			f.write(
				f'{opname}(stmt, value) :- Statement_Opcode(stmt, "{opname}"), PushValue(stmt, value).\n'
			)
		else:
	...
	f.write(
		"// Static opcode data. This file was generated by bin/generatefacts, do not edit\n\n"
	)
...
class InstructionTsvExporter(TsvExporter):
	...
	def __init__(
		self,
		output_dir: str,
		blocks: List[basicblock.EVMBasicBlock],
		ordered: bool = True,
		bytecode_hex: Optional[str] = None,
	...
	def process_metadata(self, metadata: Optional[Dict[Any, Any]] = None) -> None:
		...
		def process_function_debug_data(
			function_debug_data: Dict[str, Dict[str, Optional[int]]],
		) -> List[Tuple[str, str, int, int]]:
			return [
				(
					function_id,
					hex(debug_info["entryPoint"])
					if debug_info["entryPoint"]
					else "0x0",
		...
		def process_immutable_refs(
			immutable_refs: Dict[str, List[Dict[str, int]]],
		) -> List[Tuple[str, int]]:
			res = []
		...
		self.function_debug_data = (
			process_function_debug_data(metadata.get("function_debug_info", {}))
			if metadata is not None
			else []
		)
		self.immutable_references = (
			process_immutable_refs(metadata.get("immutable_references", {}))
			if metadata is not None
			else []
		)
	...
	def export(self):
		...
		def link_or_output_signature_file(
			signatures_filename_in: str, signatures_filename_out_simple: str
		):
			signatures_filename_out = self.get_out_file_path(
				signatures_filename_out_simple
			)
		...
		link_or_output_signature_file(
			public_function_signature_filename, "PublicFunctionSignature.facts"
		)
		...
		self.generate(
			"Statement_Next.facts", zip(instructions_order, instructions_order[1:])
		)
