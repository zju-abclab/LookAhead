...
class AnalysisExecutor:
	...
	def run_souffle_client(
		...
		if self.debug and err_file != devnull:
			...
			if any(
				s in souffle_err
				for s in ["Error", "core dumped", "Segmentation", "corrupted"]
			):
...
def write_context_depth_file(
	filename: str, max_context_depth: Optional[int] = None
) -> None:
...
class MixedFactGenerator(AbstractFactGenerator):
	...
	def add_fact_generator(
		self, pattern: str, scripts: List[str], is_default: bool, args
	):
...
class DecompilerFactGenerator(AbstractFactGenerator):
	...
	def generate_facts(
		...
		exporter.InstructionTsvExporter(
			work_dir, blocks, True, bytecode, metadata
		).export()
		...
		timeouts, _ = self.analysis_executor.run_clients(
		...
		write_context_depth_file(
			os.path.join(work_dir, "MaxContextDepth.csv"), self.context_depth
		)
	...
	def run_decomp(
		...
		config = "default"
		def_timeouts, _ = self.analysis_executor.run_clients(
			[DecompilerFactGenerator.decompiler_dl],
			[],
			in_dir,
			out_dir,
		...
		if def_timeouts or not self.decomp_out_produced(out_dir):
			if self.disable_scalable_fallback:
				...
				log(
					f"Using scalable fallback decompilation configuration for {os.path.split(contract_filename)[1]}"
				)
				write_context_depth_file(
					os.path.join(in_dir, "MaxContextDepth.csv"), 10
				)

				sca_timeouts, _ = self.analysis_executor.run_clients(
